...
def export_deck_csv(deck_id: str) -> str:
    ...
    cards = conn.execute(
        "SELECT front, back FROM flashcards WHERE deck_id = ? ORDER BY created_at", (deck_id,)
    ).fetchall()
...
def export_plan_json(plan_id: str) -> dict:
    ...
    items = conn.execute(
        "SELECT title, detail, done, sort_order FROM study_items WHERE plan_id = ? ORDER BY sort_order",
        (plan_id,),
    ).fetchall()
...
def export_full() -> dict:
    ...
    for d in decks:
        ...
        all_cards.append({"deck": dict(d), "cards": [dict(c) for c in cards]})
    ...
    for p in plans:
        ...
        all_items.append({"plan": dict(p), "items": [dict(it) for it in items]})
